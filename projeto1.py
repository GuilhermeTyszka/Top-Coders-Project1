...
def escolher_categoria(dados: list) -> str:
    ...
    '''
    categoria = input('Digite a categoria que desejada: ').replace(' ', '_').lower()
    voltar_menu = True
...
def imprimir_dados_tabela(dados: list) -> None:
    ...
    '''
    col_1 = max(len(sorted(dados, key = lambda x: len(x['id']))[-1]['id']), len('ID'))
    col_2 = max(len(sorted(dados, key = lambda x: len(x['categoria']))[-1]['categoria']), len('CATEGORIA'))
    col_3 = max(len(sorted(dados, key = lambda x: len(x['preco']))[-1]['preco']) + 3, len('PRECO'))
    ...
    espacos_col_2_dir_titulo = col_2 - len('categoria') - espacos_col_2_esq_titulo 
    ...
    print('|',' '*espacos_col_1_esq_titulo, 'ID',' '*espacos_col_1_dir_titulo,'|', ' '*espacos_col_2_esq_titulo, 'CATEGORIA', ' '*espacos_col_2_dir_titulo, '|', ' '*espacos_col_3_esq_titulo, 'PREÇO', ' '*espacos_col_3_dir_titulo, '|')
    print('+' + '-'*(col_1 + col_2 + col_3 + 15) + '+')
    for i in dados:
        ...
        print('|', ' '*espacos_col_1_esq, i['id'], ' '*espacos_col_1_dir,' '*(col_1 - len(i['id'])),'|', ' '*espacos_col_2_esq, i['categoria'].replace('_', ' ').capitalize(), ' '*espacos_col_2_dir, '|', ' '*espacos_col_3_esq, 'R$ ' + i['preco'].replace('.', ','), ' '*espacos_col_3_dir, '|')
        print('+' + '-'*(col_1 + col_2 + col_3 + 15) + '+')
        ...
        if opcao not in opcoes:
            ...
            if opcao == '1':
                ...
                for i in range(len(listar_categorias(dados)) - 1):
                    if comp_linha < 50:
                        print(listar_categorias(dados)[i].replace('_', ' ').capitalize(), end = ', ')
                        comp_linha += len(listar_categorias(dados)[i])
                    else:
                        print(listar_categorias(dados)[i].replace('_', ' ').capitalize() + ',')
                        comp_linha = 0
                print(listar_categorias(dados)[-1].replace('_', ' ').capitalize()+'.')
            elif opcao == '2':
